...
func _generate_name() -> String:
	return "MeleeAttack: %d damage, %d°" % [damage, area]
...
func _tick(delta: float) -> Status:
	...
	var target_transform: Transform3D = options.get("target", agent.transform)
	...
	params.collision_mask = Utils.get_collision_mask(agent, collision_type)
	...
	for hit in result:
		if hit.collider is HitBox:
			process_hit(hit.collider, agent)
