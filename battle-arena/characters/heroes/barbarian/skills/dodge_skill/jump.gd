...
func before_run(actor: Node, blackboard: Blackboard) -> void:
	...
	blackboard.set_value("%d_tween" % self.get_instance_id(), tween)
	blackboard.set_value("execution", time)


func tick(actor: Node, blackboard: Blackboard) -> int:
	var tween: Tween = blackboard.get_value("%d_tween" % self.get_instance_id(), null)
	if not tween: return FAILURE
	
	var time_left: float = blackboard.get_value("execution", 0.0)
	# blackboard.set_value("execution", max(time_left - get_process_delta_time(), 0.0))
	
	if tween.is_running(): return RUNNING
	blackboard.erase_value("%d_tween" % self.get_instance_id())
	blackboard.erase_value("execution")
	return SUCCESS
